...
2. **Automatic Failover**: Re-schedule agents when nodes fail
3. **Intelligent Placement**: Spread, binpack, or affinity-based scheduling
4. **Global Registry**: Single source of truth for agent state across cluster
5. **Health Monitoring**: Proactive detection and remediation
...
2. Leader sends CREATE_INSTANCE RPC to target node
3. Node's local GAPI agent manager starts agent
4. Node reports instance state back to leader
...
2. **Debugging**: Agent logs centralized and searchable
3. **Backup/Restore**: Cluster state backup and recovery
4. **Upgrades**: Rolling updates without downtime
...
2. **Design Review**: Agent spec schema and placement algorithms
3. **Prototype**: Simple spread scheduler (proof of concept)
4. **Documentation**: API design for global agent scheduling
5. **Team Alignment**: Review roadmap with stakeholders
