...
func (u *UnifiedController) FetchStatus(ctx context.Context) (_ []tui.AgentStatus, err error) {
	...
	var membersResp goblinv1.MembersResponse
	if err := client.Call("SchedulerRPC.Members", &goblinv1.MembersRequest{}, &membersResp); err != nil {
	...
	// Section 2: Jobs
	var jobsResp goblinv1.ListJobsResponse
	if err := client.Call("SchedulerRPC.ListJobs", &goblinv1.ListJobsRequest{}, &jobsResp); err != nil {
	...
	for _, job := range jobsResp.GetJobs() {
		statuses = append(statuses, tui.AgentStatus{
			ID:    job.GetJobId(),
			Type:  job.GetAgentType(),
			State: job.GetStatus(),
		})
	}

	// Section 3: Local GAPI Agents (via embedded core)
	var localResp goblinv1.ListLocalAgentsResponse
	if err := client.Call("SchedulerRPC.ListLocalAgents", &goblinv1.ListLocalAgentsRequest{}, &localResp); err != nil {
		...
		for _, agent := range localResp.GetAgents() {
			statuses = append(statuses, tui.AgentStatus{
				ID:    agent.GetId(),
				Type:  agent.GetType(),
	...
	var globalResp goblinv1.ListGlobalAgentsResponse
	if err := client.Call("SchedulerRPC.ListGlobalAgents", &goblinv1.ListGlobalAgentsRequest{}, &globalResp); err != nil {
		slog.Default().LogAttrs(ctx, slog.LevelWarn, "failed to list global agents", logattr.Err(err))
	} else {
		for _, spec := range globalResp.GetAgents() {
	...
	go func() {
		...
		if err := client.Call("SchedulerRPC.Members", &goblinv1.MembersRequest{}, &membersResp); err == nil {
			for _, m := range membersResp.GetMembers() {
				...
				ch <- fmt.Sprintf("[MEMBER] %s (%s) - %s", m.GetName(), m.GetStatus(), role)
			}
		}
		...
		if err := client.Call("SchedulerRPC.ListJobs", &goblinv1.ListJobsRequest{}, &jobsResp); err == nil {
			for _, j := range jobsResp.GetJobs() {
				ch <- fmt.Sprintf("[JOB] %s: %s (on %s)", j.GetJobId(), j.GetStatus(), j.GetAssignedNode())
			}
		}
		...
		for {
			...
			case <-ctx.Done():
				...
				return
			case <-timer.C:
				req := &goblinv1.GetEventsRequest{Cursor: lastCursor}
				var eventsResp goblinv1.GetEventsResponse

				if err := client.Call("SchedulerRPC.GetEvents", req, &eventsResp); err != nil {
					ch <- fmt.Sprintf("[ERROR] Failed to fetch events: %v", err)
				} else {
					for _, event := range eventsResp.GetEvents() {
						// Format: [HH:MM:SS.000] Message
						timestamp := event.GetTimestamp().AsTime().Format("15:04:05.000")
						ch <- fmt.Sprintf("[%s] %s", timestamp, event.GetMessage())

						if event.GetIndex() > lastCursor {
							lastCursor = event.GetIndex()
						}
					}
				}
