...
1. **Create Evidence Bundle**: For CI migration itself (implementation_plan.json, walkthrough.md)
1. **Document CI Badge**: Add status badge to README.md

### Short-term

1. **Conform AGENDA.md**: Match expected format or adapt linter
1. **Add Go Packages**: When added, verification will automatically check them
1. **Review Markdown Formatting**: Decide on enforcement policy

### Long-term

1. **Cachix Setup**: Add Cachix for faster CI builds
1. **Additional Linters**: Consider adding Go-specific linters (golangci-lint)
1. **Integration Tests**: Add scenario-level integration tests to CI
...
1. **Documentation can be outdated** - Always verify with actual tool output
1. **Nix provides excellent CI/local parity** - Worth the setup complexity
1. **Incremental migration reduces risk** - Phase large changes logically
1. **MAINTENANCE mode is critical for Runtime stabilization** - Repairing tools under `tools/` requires explicit maintenance authorization to ensure operational safety.
1. **Evidence-based debugging is faster** - Check actual output, not assumptions
